  ...
  <SEO title="Order a Pizza!" />
    <OrderStyles onSubmit={submitOrder}>
      <fieldset disabled={loading}>
        ...
        <input type="mapleSyrup" name="mapleSyrup" id="mapleSyrup" value={values.mapleSyrup} className="mapleSyrup" onChange={updateValue}/>
      </fieldset>
      <fieldset disabled={loading} className={`menu ${loading && 'disabled'}`  }>
        <legend>Menu</legend>
        {pizzas.map((pizza, i) => (
          <MenuItemStyles key={pizza.id+i}>
            ...
            <div>
              {['S','M', 'L'].map((size,i) => (
                <button key={size} type="button" onClick={() => addToOrder({
                  ...
                  {size+i}  {formatMoney(Number(calculatePizzaPrice(pizza.price, size)))}
                </button>
              ))}
            </div>
          </MenuItemStyles>
      ...
      <fieldset disabled={loading} className={`order ${loading && 'disabled'}`}>
        <legend>Order</legend>
